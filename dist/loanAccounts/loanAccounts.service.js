...
let LoanAccountsService = class LoanAccountsService {
    ...
    computeLoanStatistics(loan) {
        ...
        const totalCapital = schedules.reduce((sum, schedule) => sum + this.toNumber(schedule.capital), 0);
        const totalInterest = schedules.reduce((sum, schedule) => sum + this.toNumber(schedule.interest), 0);
        const paidCapital = schedules.reduce((sum, schedule) => sum + this.toNumber(schedule.paid_capital), 0);
        const paidInterest = schedules.reduce((sum, schedule) => sum + this.toNumber(schedule.paid_interest), 0);
        const remainingCapital = Math.max(totalCapital - paidCapital, 0);
        const remainingInterest = Math.max(totalInterest - paidInterest, 0);
        const totalFines = schedules.reduce((sum, schedule) => sum + this.toNumber(schedule.fines), 0);
        const receivingAmount = schedules.reduce((sum, schedule) => sum +
            ...
            this.toNumber(schedule.fines), 0);
    ...
    async update(id, data) {
        ...
        try {
            if (data.collector_id !== undefined) {
                if (prevCollectorId !== undefined &&
                    prevCollectorId !== data.collector_id) {
                    await this.assetManagementService.updateCollectorAssetFromLoanAccount(prevCollectorId, updated);
                }
        ...
        if (status === 'settled' || status === 'blacklist') {
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                if (rawDate) {
                    ...
                    if (dateMatch) {
                        const [, year, month, day] = dateMatch;
                        settlementDate = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day)));
                    }
                    else {
                        ...
                        settlementDate = new Date(Date.UTC(parsed.getUTCFullYear(), parsed.getUTCMonth(), parsed.getUTCDate()));
                    }
                }
                else {
                    const now = new Date();
                    settlementDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
                }
                const settlementDateStart = new Date(Date.UTC(settlementDate.getUTCFullYear(), settlementDate.getUTCMonth(), settlementDate.getUTCDate(), 0, 0, 0, 0));
                const settlementDateEnd = new Date(Date.UTC(settlementDate.getUTCFullYear(), settlementDate.getUTCMonth(), settlementDate.getUTCDate(), 23, 59, 59, 999));
                const prevTotalReceiving = schedules.reduce((sum, s) => sum + this.toNumber(s.paid_amount), 0);
                const prevPaidCapital = schedules.reduce((sum, s) => sum + this.toNumber(s.paid_capital), 0);
                const prevPaidInterest = schedules.reduce((sum, s) => sum + this.toNumber(s.paid_interest), 0);
                ...
                const manualCapitalRaw = settlement_capital !== undefined && settlement_capital !== null
                ...
                if (hasManualSettlement) {
                    ...
                    paidCapital = schedules.reduce((sum, s) => sum + this.toNumber(s.paid_capital), 0);
                }
    ...
    buildListWhereConditions(query, currentUser) {
    ...
    async findGroupedByUser(query, currentUser) {
        ...
        const { tab, isScheduleTab, whereHistory, loanAccountWhereForScheduleTabs, scheduleWhereOverdue, scheduleWhereTodayPaid, scheduleWhereTodayUnpaid, } = this.buildListWhereConditions(query, currentUser);
        ...
        const currentScheduleWhere = isScheduleTab
            ? tab === 'overdue'
                ...
                : tab === 'today_paid'
                    ...
                    : scheduleWhereTodayUnpaid
            : null;
        const [countTabHistory, countTabOverdue, countTabTodayPaid, countTabTodayUnpaid, relatedAdmins,] = await Promise.all([
